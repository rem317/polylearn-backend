...
select * from cumulative_progress;
select * from quiz_categories;



ALTER TABLE user_progress 
ADD COLUMN total_time_spent_minutes INT DEFAULT 0 	AFTER total_practice_time_seconds,
ADD INDEX idx_total_time (total_time_spent_minutes);
UPDATE user_progress 
SET total_time_spent_minutes = FLOOR(total_practice_time_seconds / 60)
WHERE total_practice_time_seconds > 0;
...
ADD COLUMN answers JSON NULL AFTER exercise_id,
ADD COLUMN max_score INT DEFAULT 0 AFTER score,
ADD COLUMN percentage INT DEFAULT 0 AFTER max_score,
ADD COLUMN attempt_number INT DEFAULT 1 AFTER completion_status,
ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP AFTER attempt_number;

ALTER TABLE practice_attempts 
ADD INDEX idx_created_at (created_at),
ADD INDEX idx_percentage (percentage);

ALTER TABLE user_activity_log 
ADD INDEX idx_user_activity_composite (user_id, activity_timestamp);


-- I-update ang lesson para i-assign sa teacher na may user_id = 4
UPDATE topic_content_items 
SET teacher_id = 4  -- ito ay user_id, hindi teacher_id mula sa teachers table
WHERE content_id = 1;

-- Tingnan kung sino ang may-ari ng lesson
SELECT 
    tci.content_id,
    tci.content_title,
    tci.created_by,
...
LEFT JOIN users teacher ON tci.teacher_id = teacher.user_id
WHERE tci.content_id = 1;

-- Tingnan ang teacher na kasalukuyang naka-login
SELECT user_id, username, full_name, role FROM users WHERE role = 'teacher';
...
ADD COLUMN time_spent_minutes INT DEFAULT 0 AFTER points_earned;

-- 1. Add created_by to topic_content_items (if not exists)
ALTER TABLE topic_content_items 
ADD COLUMN created_by INT NULL AFTER topic_id,
ADD INDEX idx_created_by (created_by);

-- 2. Add created_by to quizzes (if not exists)
ALTER TABLE quizzes
ADD COLUMN created_by INT NULL AFTER category_id,
ADD INDEX idx_quiz_created_by (created_by);

-- 3. Add created_by to practice_exercises (if not exists)
ALTER TABLE practice_exercises
ADD COLUMN created_by INT NULL AFTER topic_id,
ADD INDEX idx_practice_created_by (created_by);

-- 4. Add is_public column for sharing (optional but recommended)
ALTER TABLE topic_content_items 
ADD COLUMN is_public BOOLEAN DEFAULT TRUE AFTER created_by;
select * from topic_content_items;
...
-- 5. Add foreign keys (optional but good practice)
ALTER TABLE topic_content_items
ADD CONSTRAINT fk_lesson_created_by
...
ALTER TABLE practice_exercises
ADD CONSTRAINT fk_practice_created_by
FOREIGN KEY (created_by) REFERENCES users(user_id) ON DELETE SET NULL;
...
-- 1. Add created_at and updated_at columns (for tracking)
ALTER TABLE teachers
ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP;

-- 2. Add INDEX for better performance (optional but recommended)
ALTER TABLE teachers
ADD INDEX idx_department (department),
ADD INDEX idx_years_experience (years_experience);

-- 3. Modify qualification to have a default value (optional)
ALTER TABLE teachers
MODIFY COLUMN qualification VARCHAR(255) DEFAULT 'Licensed Professional Teacher';

-- 4. Modify department to have a default value (optional)
ALTER TABLE teachers
MODIFY COLUMN department VARCHAR(100) DEFAULT 'Mathematics';

-- Run this in MySQL once
USE polylearn_db;

-- Create trigger for automatic teacher record creation
DELIMITER //
CREATE TRIGGER after_user_insert_teacher
AFTER INSERT ON users
FOR EACH ROW
BEGIN
    -- If the new user has role 'teacher', insert into teachers table
    IF NEW.role = 'teacher' THEN
        INSERT INTO teachers (
            ...
            NEW.user_id,
            'Mathematics',  -- Default department
            'Teacher',       -- Default qualification
            0,               -- Default years experience
            CONCAT('Teacher ', NEW.full_name),  -- Default bio
            0.00,            -- Default rating
            0,               -- Default students count
            0,               -- Default lessons count
            NOW()
        );
    END IF;
END//
...
ALTER TABLE feedback
ADD COLUMN related_id INT NULL AFTER user_id,
ADD INDEX idx_related (related_id);


select * from user_quiz_attempts;

-- I-compute ang percentage base sa correct_answers
-- Kung ang total_questions = 10, ang correct_answers = 4, dapat 40%
UPDATE user_quiz_attempts 
SET score = (correct_answers / 10) * 100,
    total_questions = 10
WHERE quiz_id = 3 AND attempt_id IN (13, 14);

-- Para sa attempts na zero ang correct_answers, set sa 0
UPDATE user_quiz_attempts 
SET score = 0,
    total_questions = 10
WHERE quiz_id = 3 AND attempt_id IN (11, 12);
...
-- Add assigned_teacher_id column to quizzes table
ALTER TABLE quizzes
ADD COLUMN assigned_teacher_id INT NULL AFTER created_by,
ADD INDEX idx_quiz_assigned_teacher (assigned_teacher_id);

-- Add foreign key (optional but recommended)
ALTER TABLE quizzes
ADD CONSTRAINT fk_quiz_assigned_teacher
...
-- Add teacher reference columns to topic_content_items
ALTER TABLE topic_content_items 
ADD COLUMN teacher_id INT NULL AFTER topic_id,
ADD COLUMN created_by INT NULL AFTER teacher_id,
ADD INDEX idx_teacher (teacher_id),
ADD INDEX idx_created_by (created_by);

-- Add teacher_id to feedback table
ALTER TABLE feedback
ADD COLUMN teacher_id INT NULL AFTER user_id,
ADD INDEX idx_feedback_teacher (teacher_id);

-- Update existing lessons to associate with a teacher (run after you have teachers)
-- Example: UPDATE topic_content_items SET teacher_id = 2 WHERE topic_id IN (1,2,3);


-- 1. Sino ba talaga ang teacher? (Gamitin ang actual email mo)
SELECT user_id, username, email, role FROM users WHERE role = 'teacher';

-- 2. Ilan ang total lessons sa database?
SELECT COUNT(*) as total_lessons FROM topic_content_items;

-- 3. Ipakita ang sample lessons
SELECT 
    content_id,
    content_title,
    created_by,
...
USE polylearn_db;
SELECT user_id, username, email, role, full_name FROM users WHERE role = 'teacher';

UPDATE topic_content_items 
SET teacher_id = created_by 
WHERE created_by IN (SELECT user_id FROM users WHERE role = 'teacher')
